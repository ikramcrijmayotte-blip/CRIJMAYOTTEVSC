...
const DB_PATH = path.join(__dirname, 'crij.json');
...
if (!fs.existsSync(PUBLIC_DIR)) {
  // Dernier recours : créer le dossier
  fs.mkdirSync(PUBLIC_DIR, { recursive: true });
}
console.log('[CRIJ] Dossier public trouvé:', PUBLIC_DIR);
...
console.log('[CRIJ] Serving:', indexPath, '| existe:', fs.existsSync(indexPath));
if (!fs.existsSync(indexPath)) {
  return res.status(500).send(`
    <h2>Erreur de configuration</h2>
    <p>index.html introuvable dans : ${PUBLIC_DIR}</p>
    <p>Contenu de __dirname (${__dirname}) :</p>
    <pre>${fs.readdirSync(__dirname).join('\n')}</pre>
    <p>Contenu de process.cwd() (${process.cwd()}) :</p>
    <pre>${fs.readdirSync(process.cwd()).join('\n')}</pre>
  `);
}
res.sendFile(indexPath, err => {
  if(err) {
    console.error('[CRIJ] Erreur sendFile:', err);
